...
use ckb_vm::{
    ...
    DefaultMachineBuilder, ISA_B, ISA_IMAC, ISA_MOP,
};
use std::fs;
...
pub fn test_mop_wide_multiply() {
    ...
    let asm_core = AsmCoreMachine::new(ISA_MOP | ISA_IMAC, VERSION1, u64::max_value());
...
pub fn test_mop_ld32() {
    ...
    let program: Bytes = fs::read(program_path).unwrap().into();
    let asm_core = AsmCoreMachine::new(ISA_MOP | ISA_IMAC | ISA_B, VERSION1, u64::max_value());
    let core = DefaultMachineBuilder::<Box<AsmCoreMachine>>::new(asm_core).build();
    let mut machine = AsmMachine::new(core, None);
    machine
        .load_program(&program, &[program_path.into()])
