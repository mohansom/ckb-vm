...
static const unsigned char bf_actions[1492] = {
  ...
  1,255,72,131,252,248,240,35,1,15,133,244,247,255,72,131,252,248,0,15,132,
  244,247,72,15,189,192,72,252,247,216,72,131,192,63,255,252,233,244,248,248,
  2,255,72,99,192,255,137,192,255,185,64,0,0,0,131,226,63,41,209,72,211,224,
  137,209,255,72,211,252,248,255,72,211,232,255,72,193,224,235,255,72,193,252,
  248,235,255,72,193,232,235,255,72,137,194,240,131,185,64,0,0,0,131,226,63,
  41,209,72,211,224,137,209,255,72,199,192,237,252,233,244,13,255,72,1,135,
  233,72,139,135,233,72,57,135,233,15,134,244,247,255,72,199,135,233,237,252,
  233,245,255,72,137,135,253,240,131,233,255,72,199,194,237,232,244,11,72,131,
  252,250,0,15,133,244,247,72,141,151,233,255,136,12,2,255,102,137,12,2,255,
  72,137,12,2,255,252,233,244,248,248,1,72,137,208,252,233,244,13,248,2,255,
  72,199,194,237,232,244,12,72,131,252,250,0,15,133,244,247,72,137,194,72,129,
...
#line 1152 "src/machine/aot/aot.x64.c"
  ...
  if ((uint64_t)64 > 0xFFFFFFFF && (((uint64_t)64 & 0xFFFFFFFF80000000) != 0xFFFFFFFF80000000)) {
    loc1 = riscv_reg_to_x64_reg(target);
  dasm_put(Dst, 483, (uint64_t)64 >> 32, (uint64_t)64 & 0xFFFFFFFF);
    if (VALID_X64_REGISTER(loc1)) {
  dasm_put(Dst, 603, (loc1));
    } else {
  dasm_put(Dst, 609, Dt1(->registers[target]));
    ...
    loc1 = riscv_reg_to_x64_reg(target);
    if (VALID_X64_REGISTER(loc1)) {
  dasm_put(Dst, 783, (loc1), (uint64_t)64);
    } else {
  dasm_put(Dst, 790, Dt1(->registers[target]), (uint64_t)64);
    }
  }
...
#line 1211 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      //| shl rax, cl
      //| mov ecx, edx
      dasm_put(Dst, 1265, (bits.value.x64_reg));
...
#line 1250 "src/machine/aot/aot.x64.c"
  ...
  //| jna >1
  dasm_put(Dst, 1295, Dt1(->cycles), Dt1(->max_cycles), Dt1(->cycles));
...
#line 1306 "src/machine/aot/aot.x64.c"
        ...
        case 0x40:
          ...
          dasm_put(Dst, 1312, Dt1(->pc), ((uint32_t)(value.value.i & 0x7FFFFFFF)), ((value.value.i >> 32) ^ 0x40000000));
...
#line 1314 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      dasm_put(Dst, 1321, (value.value.x64_reg), Dt1(->pc));
...
int aot_memory_write(AotContext* context, AotValue address, AotValue v, uint32_t size)
{
  ...
  dasm_put(Dst, 1329, size, Dt1(->memory));
...
int aot_memory_read(AotContext* context, uint32_t target, AotValue address, uint32_t size)
{
  ...
  dasm_put(Dst, 1380, size, size, CKB_VM_ASM_RISCV_MAX_MEMORY);
...
static int aot_mov_internal(AotContext* context, riscv_register_t target, AotValue value, x64_register_t x64_temp_reg)
{
  ...
  switch (value.tag) {
    case AOT_TAG_REGISTER:
      ...
      if (VALID_X64_REGISTER(loc1) && VALID_X64_REGISTER(loc2)) {
      dasm_put(Dst, 1442, (loc2), (loc1));
      } else if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1450, (loc1), Dt1(->registers[value.value.reg]));
      } else if (VALID_X64_REGISTER(loc2)) {
      dasm_put(Dst, 1321, (loc2), Dt1(->registers[target]));
      } else {
      dasm_put(Dst, 1458, (x64_temp_reg), Dt1(->registers[value.value.reg]), (x64_temp_reg), Dt1(->registers[target]));
      }
#line 1463 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_IMMEDIATE:
      ...
      if (value.value.i > 0xFFFFFFFF && ((value.value.i & 0xFFFFFFFF80000000) != 0xFFFFFFFF80000000)) {
        loc1 = riscv_reg_to_x64_reg(target);
      dasm_put(Dst, 1473, (x64_temp_reg), value.value.i >> 32, (x64_temp_reg), (x64_temp_reg), value.value.i & 0xFFFFFFFF);
        if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1442, (x64_temp_reg), (loc1));
        } else {
      dasm_put(Dst, 1321, (x64_temp_reg), Dt1(->registers[target]));
        }
      } else {
        loc1 = riscv_reg_to_x64_reg(target);
        if (VALID_X64_REGISTER(loc1)) {
...
#line 1466 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      loc1 = riscv_reg_to_x64_reg(target);
      if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1442, (value.value.x64_reg), (loc1));
      } else {
      dasm_put(Dst, 1321, (value.value.x64_reg), Dt1(->registers[target]));
      }
...
static int aot_mov_x64(AotContext* context, x64_register_t x64_target, AotValue value)
{
  ...
  switch (value.tag) {
    case AOT_TAG_REGISTER:
      ...
      if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1442, (loc1), (x64_target));
      } else {
      dasm_put(Dst, 1450, (x64_target), Dt1(->registers[value.value.reg]));
      }
#line 1482 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_IMMEDIATE:
      ...
      if (value.value.i > 0xFFFFFFFF && ((value.value.i & 0xFFFFFFFF80000000) != 0xFFFFFFFF80000000)) {
      dasm_put(Dst, 1473, (x64_target), value.value.i >> 32, (x64_target), (x64_target), value.value.i & 0xFFFFFFFF);
      } else {
...
#line 1485 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      dasm_put(Dst, 1442, (value.value.x64_reg), (x64_target));
