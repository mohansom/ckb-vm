...
static const unsigned char bf_actions[1598] = {
  ...
  15,184,192,255,128,225,127,128,252,249,63,15,142,244,247,128,252,233,64,72,
  49,208,72,49,194,72,49,208,248,1,128,252,249,0,15,133,244,248,252,233,244,
  249,248,2,72,211,224,252,246,217,128,193,64,72,211,252,234,72,9,208,248,3,
  255,128,225,127,128,252,249,63,15,142,244,247,128,252,233,64,72,49,208,72,
  49,194,72,49,208,248,1,128,252,249,0,15,133,244,248,252,233,244,249,248,2,
  72,211,232,252,246,217,128,193,64,72,211,226,72,9,208,248,3,255,72,99,192,
  ...
  137,135,253,240,131,233,255,72,199,194,237,232,244,11,72,131,252,250,0,15,
  133,244,247,72,141,151,233,255,136,12,2,255,102,137,12,2,255,72,137,12,2,
  255,252,233,244,248,248,1,72,137,208,252,233,244,13,248,2,255,72,199,194,
  237,232,244,12,72,131,252,250,0,15,133,244,247,72,137,194,72,129,194,239,
...
int aot_fsl(AotContext* context, riscv_register_t target, AotValue a, AotValue b, AotValue c)
{
  ...
  switch (c.tag) {
    case AOT_TAG_REGISTER:
      ...
      dasm_put(Dst, 1013, Dt1(->registers[c.value.reg]));
      }
#line 1274 "src/machine/aot/aot.x64.c"
      break;
    case AOT_TAG_IMMEDIATE:
      //| mov ecx, c.value.i
      dasm_put(Dst, 1018, c.value.i);
#line 1277 "src/machine/aot/aot.x64.c"
      break;
    case AOT_TAG_X64_REGISTER:
      //| mov rcx, Rq(c.value.x64_reg)
      dasm_put(Dst, 1007, (c.value.x64_reg));
...
#line 1402 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      //| shl rax, cl
      //| mov ecx, edx
      dasm_put(Dst, 1383, (bits.value.x64_reg));
...
#line 1441 "src/machine/aot/aot.x64.c"
  ...
  //| jna >1
  dasm_put(Dst, 1413, Dt1(->cycles), Dt1(->max_cycles), Dt1(->cycles));
...
#line 1502 "src/machine/aot/aot.x64.c"
        ...
        case 0x40:
          ...
          dasm_put(Dst, 1430, Dt1(->pc), ((uint32_t)(value.value.i & 0x7FFFFFFF)), ((value.value.i >> 32) ^ 0x40000000));
...
#line 1510 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      dasm_put(Dst, 1439, (value.value.x64_reg), Dt1(->pc));
...
int aot_memory_write(AotContext* context, AotValue address, AotValue v, uint32_t size)
{
  ...
  dasm_put(Dst, 1447, size, Dt1(->memory));
...
int aot_memory_read(AotContext* context, uint32_t target, AotValue address, uint32_t size)
{
  ...
  dasm_put(Dst, 1498, size, size, CKB_VM_ASM_RISCV_MAX_MEMORY);
...
static int aot_mov_internal(AotContext* context, riscv_register_t target, AotValue value, x64_register_t x64_temp_reg)
{
  ...
  switch (value.tag) {
    case AOT_TAG_REGISTER:
      ...
      if (VALID_X64_REGISTER(loc1) && VALID_X64_REGISTER(loc2)) {
      dasm_put(Dst, 1560, (loc2), (loc1));
      } else if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1568, (loc1), Dt1(->registers[value.value.reg]));
      } else if (VALID_X64_REGISTER(loc2)) {
      dasm_put(Dst, 1439, (loc2), Dt1(->registers[target]));
      } else {
      dasm_put(Dst, 1576, (x64_temp_reg), Dt1(->registers[value.value.reg]), (x64_temp_reg), Dt1(->registers[target]));
      }
#line 1659 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_IMMEDIATE:
      ...
      if (value.value.i > 0xFFFFFFFF && ((value.value.i & 0xFFFFFFFF80000000) != 0xFFFFFFFF80000000)) {
        loc1 = riscv_reg_to_x64_reg(target);
      dasm_put(Dst, 1591, (x64_temp_reg), (unsigned int)(value.value.i), (unsigned int)((value.value.i)>>32));
        if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1560, (x64_temp_reg), (loc1));
        } else {
      dasm_put(Dst, 1439, (x64_temp_reg), Dt1(->registers[target]));
        }
      } else {
        loc1 = riscv_reg_to_x64_reg(target);
        if (VALID_X64_REGISTER(loc1)) {
...
#line 1662 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      loc1 = riscv_reg_to_x64_reg(target);
      if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1560, (value.value.x64_reg), (loc1));
      } else {
      dasm_put(Dst, 1439, (value.value.x64_reg), Dt1(->registers[target]));
      }
...
static int aot_mov_x64(AotContext* context, x64_register_t x64_target, AotValue value)
{
  ...
  switch (value.tag) {
    case AOT_TAG_REGISTER:
      ...
      if (VALID_X64_REGISTER(loc1)) {
      dasm_put(Dst, 1560, (loc1), (x64_target));
      } else {
      dasm_put(Dst, 1568, (x64_target), Dt1(->registers[value.value.reg]));
      }
#line 1678 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_IMMEDIATE:
      ...
      if (value.value.i > 0xFFFFFFFF && ((value.value.i & 0xFFFFFFFF80000000) != 0xFFFFFFFF80000000)) {
      dasm_put(Dst, 1591, (x64_target), (unsigned int)(value.value.i), (unsigned int)((value.value.i)>>32));
      } else {
...
#line 1681 "src/machine/aot/aot.x64.c"
    ...
    case AOT_TAG_X64_REGISTER:
      ...
      dasm_put(Dst, 1560, (value.value.x64_reg), (x64_target));
