use super::{
    super::{decoder::build_decoder, instructions::Register, memory::Memory, Error},
    CoreMachine, DefaultMachine, Machine, SupportMachine,
};
use bytes::Bytes;

pub trait PProfLogger<Mac> {
    fn step(&mut self, machine: &mut Mac);
    fn stop(&mut self, machine: &mut Mac);
}
    ...
    PProfMachine<'a, Inner>
{
    ...
    pub fn run(&mut self) -> Result<i8, Error> {
        ...
        while self.machine.running() {
            self.pprof_logger.step(&mut self.machine);
            self.machine.step(&decoder)?;
        }
        self.pprof_logger.stop(&mut self.machine);
